...
Accepted papers will be published by Springer (with appropriate copyrights) electronically up to \textcolor{red}{two weeks prior} to the main conference.
...
Publication of the paper in the ECCV \ECCVyear{} proceedings of Springer requires that at least one of the authors register for the conference \textcolor{red}{at the full rate} and present the paper there. 
...
To do so, Springer needs all source files (\LaTeX{} files with all the associated style files, underlying \texttt{.bib} file for the references, special fonts, and PDF/EPS/JPG/PNG files for the figures, or Word/RTF files) and the PDFs of the paper. 
...
For each paper, one ZIP file called XXXXX.ZIP (where XXXXX is the zero-padded, five-digit paper ID) has to be prepared and submitted via the ECCV \ECCVyear{} Submission Website, using the password you received with your initial registration on that site. 
The size of the ZIP file may not exceed the limit of \textcolor{red}{50 MByte}. 
The ZIP file has to contain the following:
  \begin{enumerate}
 \item All source files, \eg, \LaTeX{}2$_\varepsilon$ files for the text, PS/EPS or PDF/JPG/PNG files for all figures.
 \item PDF file named ``XXXXX.pdf'' that has been produced by the submitted source, where XXXXX is the five-digit paper ID (zero-padded if necessary). For example, if your paper ID is 24, the filename must be 00024.pdf. 
...
Please ensure that you upload the correct camera-ready PDF (\ie, renamed to XXXXX.pdf) as described in the previous step of your camera-ready submission. 
...
Further considerations for preparing the camera-ready package:
  \begin{enumerate}
    ...
    \item Please do not send any older versions of papers. 
          ...
          Springer's typesetters require the author-created PDFs in order to check the proper representation of symbols, figures, \etc
...
\begin{itemize}
  ...
  \item I have not used the \verb|\cite| command in the abstract.
  ...
  \item I have inserted the ORCID identifiers of the authors in the paper header (see \url{http://bit.ly/2H5xBpN} for more information).
